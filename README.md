...
- Desktop deck cards expose native WAV loading, native play/pause/stop, Deck A/B endpoint capture controls, driverless app process capture, and a quick tone test for the native engine.
...
Desktop deck processing includes per-deck pan, DJ filter, per-deck EQ curves, per-deck EQ bypass, DJ effect-chain state, and optional advanced plugin-chain state. These settings are sent to the Electron audio engine and persisted. Local WAV decks, pushed PCM, bounded capture buffers, continuous Deck A/B capture streams, and experimental Microsoft WASAPI process-loopback capture can now flow through the native persistent router without requiring a virtual driver. When Windows reports a Resonance virtual capture endpoint, the desktop engine can select it as a capture input, but the main product path is app-owned and driverless captured audio first. Active staged DJ effects feed a built-in NativeDSP lane, and the first staged bridge-capable VST3 plugin per deck can receive live Deck A/B PCM through the native bridge with NativeDSP fallback when loading, processing, or silence checks fail.
