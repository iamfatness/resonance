...
stdin JSON: {"type":"processPcm","pluginId":"...","frames":512,"channels":2,"sampleRate":48000,"pcm16File":"C:\\Temp\\in.pcm16","outputPcm16File":"C:\\Temp\\out.pcm16"}
stdin JSON: {"type":"unloadPlugin","pluginId":"..."}
stdin JSON: {"type":"exit"}
```

`processPcm` keeps JSON/base64 PCM as the default compatibility transport. For lower JSON overhead during native router playback, set `RESONANCE_VST3_PCM_TRANSPORT=file` or `RESONANCE_VST3_PCM_FILE_TRANSPORT=1`; the router then writes raw interleaved little-endian PCM16 blocks to temporary files, sends `pcm16File`/`outputPcm16File` paths to the bridge, reads the processed output file, and deletes both files after the block. This is an opt-in stepping stone toward shared-memory or a dedicated binary block protocol.
...
App-owned Deck A/B PCM, local files, or capture streams
  ...
  -> Per-deck VST3 bridge path when validated
  -> Master summing bus
  -> WASAPI render output

Optional later:
System audio -> Resonance virtual playback device -> Deck A/B capture
```

## Native Host Milestones

1. Keep app-owned Deck A/B PCM as the primary plugin-host input path.
...
4. Improve latency by validating the opt-in PCM file transport, then move to shared-memory or a dedicated binary block protocol if needed.
