...
- The desktop engine receives `pluginChain` settings over IPC.
...
Resonance virtual playback device
  -> WASAPI capture
  -> Resonance app EQ, if not bypassed
  -> VST3/Waves plugin chain
  -> WASAPI render output
```

## Native Host Milestones

1. Capture real PCM from the virtual playback device.
2. Render processed PCM to the selected Windows output.
3. Add VST3 plugin discovery for common install paths.
4. Load one plugin instance in-process or through a sandboxed helper.
5. Add plugin parameter state, bypass, ordering, and preset persistence.
6. Validate Waves plugins specifically after the generic VST3 path works.
