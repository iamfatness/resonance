...
The renderer merges scanned desktop candidates into the plugin catalog so users can stage discovered VST2/VST3 candidates on Deck A or Deck B. VST3 candidates can expose real parameter controls when the native bridge reports `status: loaded`, and the bridge can call `processTone` or `processPcm` to run test blocks through that processor. They remain `blocked-third-party` for live deck audio unless the native bridge reports `processingEnabled: true`, which is intentionally held false until continuous Deck A/B PCM streaming is wired into the VST3 process path.
...
3. Connect the persistent router render callback to the verified VST3 bridge `processPcm` block exchange.
