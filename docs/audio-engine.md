...
The renderer surfaces this state in two places: a compact native source/VST3 summary on each desktop deck card, and the expanded Desktop Audio Engine settings panel with detailed Deck A/B bus meters, source labels, route labels, and VST3 bridge/fallback status. The YouTube iframe deck remains labeled as browser-isolated because that browser path is not the native processable source.
