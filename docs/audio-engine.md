...
The renderer merges scanned desktop candidates into the plugin catalog so users can stage discovered VST2/VST3 candidates on Deck A or Deck B. VST3 candidates can expose real parameter controls when the native bridge reports `status: loaded`, and the bridge can call `processTone` or `processPcm` to run test blocks through that processor. During persistent desktop playback, the audio router selects the first staged bridge-capable VST3 plugin on each deck, keeps a per-deck bridge process warm, and routes live deck PCM blocks through `processPcm` before pan/mix/output. If the bridge fails or returns silence for non-silent input, the router falls back to the built-in NativeDSP lane.
...
3. Validate audible live output with a known non-silent VST3 effect, then forward parameter changes into the loaded VST3 controller.
